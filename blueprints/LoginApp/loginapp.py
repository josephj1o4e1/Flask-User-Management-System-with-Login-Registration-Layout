...
# decorator @login_bp.route('/'): before triggering home(), we need to detect if url '/' is requested by client, and later do the login confirmation before executing home().  
@login_bp.route('/')
@login_required
@check_confirmed
def home():
...
@login_bp.route('/google_login/authorized')
...
@login_bp.route('/github_login')
def github_login():
    return render_template("notfinished.html")

@login_bp.route('/github_login/authorized')
def github_authorized():
    pass

@login_bp.route('/orcid_login')
def orcid_login():
    return render_template("notfinished.html")

@login_bp.route('/orcid_login/authorized')
def orcid_authorized():
    pass

@login_bp.route('/facebook_login')
def facebook_login():
    return render_template("notfinished.html")

@login_bp.route('/facebook_login/authorized')
def facebook_authorized():
    pass


@login_bp.route('/unconfirmed')
@login_required
def unconfirmed():
    next = request.args.get('next')
...
@login_bp.route('/delete_account', methods=['POST'])
